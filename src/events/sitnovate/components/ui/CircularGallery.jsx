...
function createTextTexture(gl, text, font = 'bold 30px monospace', color = 'black') {
    ...
    const lineHeight = parseInt(font, 10) * 1.3;
    ...
    canvas.width = (maxWidth + 40) * dpr;
    canvas.height = (totalHeight + 40) * dpr;
    
    // Scale context for high DPI
    context.scale(dpr, dpr);
    
    // Re-set font and improve rendering
    context.font = font;
...
class Title {
    ...
    createMesh() {
      ...
      `,
        ...
        const textHeight = this.plane.scale.y * 0.3;
...
class Media {
    ...
    createShader() {
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          // Removed wave effect - keep cards flat
...
class App {
    ...
    onTouchMove(e) {
        ...
        if (Math.abs(x - this.clickStartX) > 5) {
            this.hasMoved = true;
        }
    }
    onTouchUp(e) {
        ...
        // Only trigger click if user hasn't moved much and it was a quick tap
        if (this.clickStartTime && 
            Date.now() - this.clickStartTime < 300 && 
            !this.hasMoved &&
    ...
    handleClick(x, y) {
        ...
        if (this.raycast) {
            ...
            mouse.x = (relX / rect.width) * 2 - 1;
            mouse.y = -(relY / rect.height) * 2 + 1;
    ...
    addEventListeners() {
        ...
        this.gl.canvas.addEventListener('dblclick', this.boundOnClick);
    ...
    destroy() {
        ...
        if (this.gl && this.gl.canvas) {
            ...
            this.gl.canvas.removeEventListener('dblclick', this.boundOnClick);
        }
